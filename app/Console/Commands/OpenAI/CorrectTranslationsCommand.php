...
class CorrectTranslationsCommand extends Command
{
    protected $signature = 'openai:correct-translations
                            {--input= : Input CSV file path (default: app/Console/Commands/FN3/combined_lus.csv)}
                            {--output= : Output CSV file path (default: app/Console/Commands/FN3/combined_lus_corrected.csv)}
    ...
    protected $description = 'Evaluate and correct Portuguese translations of lexical units using OpenAI';
    ...
    public function handle(): int
    {
        $inputPath = $this->option('input')
            ?? app_path('Console/Commands/FN3/combined_lus.csv');
        $outputPath = $this->option('output')
            ?? app_path('Console/Commands/FN3/combined_lus_corrected.csv');
        $batchSize = (int) $this->option('batch-size');
        $resume = $this->option('resume');
        $dryRun = $this->option('dry-run');
        $model = $this->option('model');

        $this->progressFile = dirname($outputPath).'/correction_progress.json';
            ...
            for ($i = $startIndex; $i < $endIndex; $i++) {
                ...
                try {
                    ...
                    if ($result['corrected']) {
                        ...
                        if ($dryRun) {
                            $this->newLine();
                            $this->warn("Row {$i}: {$row['lemmaName(en)']}");
    ...
    private function evaluateTranslation(array $row, string $model, string $apiKey): array
    {
        ...
        $idPos = (int) $row['idPOS'];
        $senseDescription = $row['senseDescription(pt)'];

        $posName = $this->posMap[$idPos] ?? 'UNKNOWN';

        $prompt = $this->buildPrompt($lemmaEn, $lemmaPt, $posName, $senseDescription);
    ...
    private function buildPrompt(string $lemmaEn, string $lemmaPt, string $posName, string $senseDescription): string
    {
        return <<<PROMPT
...
Sense description (in Portuguese): "{$senseDescription}"

Check if the Portuguese translation is correct by verifying:
1. The word matches the meaning described in the sense description
...
PROMPT;
    ...
    private function generateOutputCsv(string $outputPath, array $rows, array $processedData): void
    {
        ...
        // Write header
        $headers = ['lemmaName(en)', 'lemmaName(pt)', 'idPOS', 'senseDescription(pt)', 'idLU', 'source_file'];
