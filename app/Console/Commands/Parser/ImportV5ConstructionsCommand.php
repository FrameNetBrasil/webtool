...
/**
 * Import V5 Construction Definitions
 *
 * Imports construction patterns from JSON files into parser_construction_v4 table
 * with V5 extensions (mandatoryElements, optionalElements, ghostCreationRules).
 *
 * Files:
 ...
 *
 * Usage:
 *   php artisan parser:import-v5-constructions --grammar=1
 *   php artisan parser:import-v5-constructions --grammar=1 --clear
 *   php artisan parser:import-v5-constructions --grammar=1 --data-path=resources/data/v5
 */
class ImportV5ConstructionsCommand extends Command
{
    protected $signature = 'parser:import-v5-constructions
                            {--grammar=1 : Grammar graph ID}
                            {--clear : Clear existing constructions before import}
                            {--dry-run : Show what would be imported without actually importing}
                            {--data-path=resources/data/v5 : Path to construction JSON files}';

    protected $description = 'Import V5 construction definitions from JSON files with ghost node support';
    ...
    public function handle(): int
    {
        ...
        $this->info('Parser V5 Construction Import');
        ...
        if (! $this->verifyV5Schema($dryRun)) {
            $this->error('V5 schema extensions not found. Please run the V5 migration first.');
    ...
    private function verifyV5Schema(bool $dryRun): bool
    {
        ...
        if (! empty($missingColumns)) {
            $this->warn('Missing V5 columns: '.implode(', ', $missingColumns));

            return false;
        }

        $this->info('V5 schema extensions verified');
