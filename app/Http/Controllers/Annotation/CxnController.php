...
{
    #[Get(path: '/annotation/cxn')]
    public function browse(SearchData $search)
    {
        $data = BrowseService::browseCorpusBySearch($search, [], 'CorpusAnnotation');
        session(["corpusAnnotationType" =>"cxn"]);
        return view('Annotation.browseSentences', [
            'page' => 'Construction Annotation',
            'url' => '/annotation/cxn/sentence',
            'data' => $data,
        ]);
    }

    #[Get(path: '/annotation/cxn/sentence/{idDocumentSentence}/{idAnnotationSet?}')]
    public function annotation(int $idDocumentSentence, ?int $idAnnotationSet = null)
    {
        $data = CorpusService::getResourceData($idDocumentSentence, $idAnnotationSet, 'cxn');
        $page = 'Construction Annotation';
        $url = '/annotation/cxn/sentence';
