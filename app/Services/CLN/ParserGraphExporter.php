...
use App\Models\CLN\Pathway;
use App\Models\CLN\PathwayDirection;
...
class ParserGraphExporter
{
     ...
     */
    public function exportToDot(RuntimeGraph $graph, string $sentence, array $stats = [], array $activeNodes = []): string
    {
        $dot = [];
        ...
        // Export edges
        $dot[] = '  // Edges';
        $allNodes = $graph->getAllColumns();
        $edgeCount = 0;

//        foreach ($allNodes as $node) {
            $edges = $graph->getAllPathways();
...
//        }
     ...
     */
    private function formatNode(FunctionalColumn $node, array $activeNodes): string
    {
...
//        }
     ...
     */
    private function buildNodeLabel(FunctionalColumn $node): string
    {
        $parts = [];
...
//        }
     ...
     */
    private function getNodeColor(FunctionalColumn $node, array $activeNodes): string
    {
        $activation = $node->L5->activation;
        if ($activation > 0) {
...
//        }
     ...
     */
    private function getNodeShape(FunctionalColumn $node): string
    {
//        if ($node->getLayer() === 'L1') {
//            if ($node->type === 'pos') {
//                return 'hexagon';
...
//        }
     ...
     */
    private function formatEdge(Pathway $edge): string
    {
        $source = $this->escapeId($edge->source->id);
        $target = $this->escapeId($edge->target->id);
        $direction = $edge->direction;
        if ($direction == PathwayDirection::FEEDBACK) {
            $color = 'red';
        }
        if ($direction == PathwayDirection::FEEDFORWARD) {
            $color = 'black';
        }
        if ($direction == PathwayDirection::LATERAL) {
            $color = 'blue';
        }
