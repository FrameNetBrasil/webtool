 ...
 * - Mandatory: <element> (required, can create ghost nodes)
...
class PatternCompiler
{
    ...
    public function tokenize(string $pattern): array
    {
        ...
        while ($i < $length) {
            ...
            // Mandatory <...> (can create ghost nodes)
            if ($char === '<') {
                $content = $this->extractBracketed($pattern, $i, '<', '>');
                $tokens[] = [
                    'type' => 'MANDATORY',
     ...
     * Required element that can be fulfilled by ghost node if needed
     */
