...
class ValenceService extends MService
{
    ...
    public function getFramePatterns($idLanguage, $idFrame = '')
    {
        ...
        foreach ($frames as $idFrame) {
            $lus = $this->getLUs($idFrame, $idLanguage);
    ...
    public function getLUs($idFrame, $idLanguage)
    {
...
";
        ...
        if (count($result) > 0) {
            ...
            foreach ($result as $row) {
                ...
                if (($row['startChar'] != '') && (($row['startChar'] != $startChar) || ($row['layerEntry'] == 'lty_fe'))) { // comecou novo valente
                    if ($row['layerEntry'] != 'lty_fe') continue;
                    if (($row['feTypeEntry'] != 'cty_core') && ($row['feTypeEntry'] != 'cty_core-unexpressed')) continue;
                    $feName = $row['feName'];
                    $feId = $row['feId'];
                    $valencia[$anId][$feId] = [];
                    $startChar = $row['startChar'];
...
" . ($idFrame != '' ? "and (idFrame IN ({$idFrame}))" : "");
        $rows = $db->getQueryCommand($query)->getResult();
        foreach ($rows as $row) {
            $transaction = $db->beginTransaction();
            try {
                $idFrame = $row['idFrame'];
                $this->clearValence($idFrame, $idLanguage);
                $result = $this->getFramePatterns($idLanguage, $idFrame);
                mdump($result);
                foreach ($result as $idLU => $patterns) {
                    $command = "insert into ValenceLU (idFrame, idLU, idLanguage) values({$idFrame}, {$idLU}, {$idLanguage})";
                    $db->executeCommand($command);
                    $idValenceLU = $db->lastInsertId();
                    foreach ($patterns as $idPattern => $pattern) {
                        //mdump($idPattern);
                        $count = $pattern['count'];
                        $command = "insert into ValencePattern (idValenceLU, countPattern) values({$idValenceLU}, {$count})";
                        $db->executeCommand($command);
                        $idValencePattern = $db->lastInsertId();
                        foreach ($pattern['valence'] as $idFE => $valence) {
                            //mdump($valence);
                            $command = "insert into ValenceValent (idValencePattern, idFrameElement, GF, GFSource, PT) values({$idValencePattern}, {$idFE}, '{$valence['GF']}', '{$valence['GFSource']}', '{$valence['PT']}')";
                            $db->executeCommand($command);
                        }
                    }
                }
    ...
    public function clearValence($idFrame, $idLanguage) {
        $v = new fnbr\models\ViewFrame();
        $db = $v->getDb();
        $command = "delete from ValenceValent where idValencePattern IN (select idValencePattern from ValencePattern where idValenceLU in (select idValenceLU from ValenceLU where (idFrame = {$idFrame}) and (idLanguage = {$idLanguage})))";
        $db->executeCommand($command);
        $command = "delete from ValencePattern where idValenceLU in (select idValenceLU from ValenceLU where (idFrame = {$idFrame}) and (idLanguage = {$idLanguage}))";
        $db->executeCommand($command);
        $command = "delete from ValenceLU where (idFrame = {$idFrame}) and (idLanguage = {$idLanguage})";
        $db->executeCommand($command);
    }
